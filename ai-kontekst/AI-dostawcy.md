...
**Źródło prawdy w kodzie:** `packages/api/ai/cascade-config.ts` → `MODEL_CASCADE`. Ten dokument ma być
zsynchronizowany z kodem.
...
> **OpenRouter** — kod wspiera (`OPENROUTER_API_KEY`), ale brak slotów w kaskadzie. Dodaj w `cascade-config.ts` jeśli potrzebujesz.
